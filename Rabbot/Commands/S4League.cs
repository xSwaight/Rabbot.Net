...
namespace Rabbot.Commands
{
    public class S4League : InteractiveBase
    {
        ...
        [BotCommand]
        [Summary("Zeigt Statistiken von S4 Remnants oder S4 League Official an.")]
        [Command("s4stats")]
        public async Task S4Stats()
        {
        ...
        private async Task SetRemnantsStats(SocketCommandContext context, IUserMessage message)
        {
            using (var db = Database.Open())
            {
                await message.ModifyAsync(p => { p.Content = "Berechne S4 Remnants Stats..."; p.Embed = null; });
                //Remnants 
                ...
                var remnantsPlayerPeak = db.RemnantsPlayers.AsQueryable().FirstOrDefault(p => p.Playercount == db.RemnantsPlayers.Max(p => p.Playercount));
                var remnantsFirstDate = db.RemnantsPlayers.AsQueryable().FirstOrDefault(p => p.Date == db.RemnantsPlayers.Min(p => p.Date));
                var remnantsLastDate = db.RemnantsPlayers.AsQueryable().FirstOrDefault(p => p.Date == db.RemnantsPlayers.Max(p => p.Date));
                ...
                await message.ModifyAsync(p => { p.Content = "**S4 Remnants Stats**"; p.Embed = embedRemnants.Build(); });
            }
        }

        private async Task SetOfficialStats (SocketCommandContext context, IUserMessage message)
        {
            using (var db = Database.Open())
            {
                await message.ModifyAsync(p => { p.Content = "Berechne S4 Official Stats..."; p.Embed = null; });
                ...
                var officialPlayerPeak = db.OfficialPlayers.AsQueryable().FirstOrDefault(p => p.Playercount == db.OfficialPlayers.Max(p => p.Playercount));
                var officialFirstDate = db.OfficialPlayers.AsQueryable().FirstOrDefault(p => p.Date == db.OfficialPlayers.Min(p => p.Date));
                var officialLastDate = db.OfficialPlayers.AsQueryable().FirstOrDefault(p => p.Date == db.OfficialPlayers.Max(p => p.Date));
                ...
                await message.ModifyAsync(p => { p.Content = "**S4 Official Stats**"; p.Embed = embedOfficial.Build(); });
            }
        }
    }
}
